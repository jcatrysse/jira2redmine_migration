...
const MIGRATE_PROJECTS_SCRIPT_VERSION = '0.0.4';
...
function main(array $config, array $cliOptions): void
{
    ...
    if (in_array('transform', $phasesToRun, true)) {
        ...
        printf(
            "[%s] Completed transform phase. Matched: %d, Ready: %d, Manual: %d, Skipped: %d, Unchanged: %d.%s",
            formatCurrentTimestamp(),
...
/**
 * @return array{matched: int, ready_for_creation: int, manual_review: int, skipped: int, unchanged: int, status_counts: array<string, int>}
 */
function runProjectTransformationPhase(PDO $pdo): array
{
    ...
    foreach ($mappings as $row) {
        ...
        $notes = null;
        $redmineProjectId = $row['redmine_project_id'] !== null ? (int)$row['redmine_project_id'] : null;
        ...
        if ($projectKey === null || $projectKey === '') {
            ...
            $notes = 'Missing Jira project key in the staging snapshot. Re-run extraction or update manually.';
        } else {
            ...
            if ($normalizedIdentifier === null) {
                $newStatus = 'MANUAL_INTERVENTION_REQUIRED';
                $notes = sprintf('Unable to derive a valid Redmine identifier from Jira key "%s".', $projectKey);
            } else {
                $matchedProject = $redmineLookup[$normalizedIdentifier] ?? null;

                if ($matchedProject !== null) {
                    $newStatus = 'MATCH_FOUND';
                    $redmineProjectId = (int)$matchedProject['id'];
                } else {
...
/**
 * @return array<string, array{id: int, identifier: string, name: ?string}>
 */
...
/**
 * @param array<int, array{mapping_id: int, jira_project_id: string, project_key: ?string, project_name: ?string, description: ?string, is_private: ?int}> $operations
 */
...
/**
 * @param array{mapping_id: int, jira_project_id: string, project_key: ?string, project_name: ?string, description: ?string, is_private: ?int} $operation
...
function buildProjectCreationPayload(array $operation): array
{
    ...
    $name = $projectName ?? $projectKey ?? $operation['jira_project_id'];
    $name = normalizeString($name, 255) ?? $identifier;

    $description = normalizeMultilineString($operation['description']);
