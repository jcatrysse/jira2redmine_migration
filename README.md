...
| `--phases=<list>` | Comma-separated list of phases to run (e.g., `jira`, `redmine`, `transform`). |
| `--skip=<list>` | Comma-separated list of phases to skip. |

**Default behaviour:** when no phase-related options are supplied, all three phases (`jira`, `redmine`, and `transform`) are executed in order.
...
# Recalculate the mapping & proposed values without hitting any APIs
